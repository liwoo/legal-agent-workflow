...
#
# `make up` starts the Langfuse + MinIO containers (detached), then runs the
# three app processes — DevUI (:8080), API (:8000), frontend (:3000) — in the
...
.PHONY: help install preflight infra up down clean e2e ps logs

help:
	@echo "Contract Triage — make targets:"
	@echo "  make install   one-time: Python venv + frontend deps"
	@echo "  make up         bring up the whole stack (containers + DevUI + API + frontend)"
